...
/* defines encoder mode */
typedef enum VIDEO_ENC_MODE
{
    ENC_MODE_DEF      = 0,          ///< default
    ENC_MODE_FAST_DEC = 1           ///< favors fast decode
} VIDEO_ENC_MODE;
...
} video_enc_priv_data_VP8;

/* defines codec specific data */
typedef union video_enc_codec_priv
{
    video_enc_priv_data_VP8 vp8;    ///< VP8 codec specifics
} video_enc_codec_priv;
...
typedef struct video_enc_frame_bs
{
    ...
    long long pts;                  ///< 'pts' in the video_surface
    int       flags;                ///< frame type etc.
    double    psnr;                 ///< frame quality: ~30
    long long opaque;               ///< 'opaque' in the video_surface

    int       codec;                ///< codec
    video_enc_codec_priv codec_priv;///< codec-private data
} video_enc_frame_bs;

/* encoded frame callback */
typedef void(*cb_enc_output)(void * enc, video_enc_frame_bs * frame, void * cb_arg);
